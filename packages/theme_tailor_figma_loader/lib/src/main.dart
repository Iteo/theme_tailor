...
import 'templates/class_color_template.dart';
import 'templates/class_typography_template.dart';
...
Future<void> handleStyleFiles(GenerateOptions options) async {
  ...
  final outputColorsPath =
      Directory(path.join(current.path, output.path, options.outputFileColors));
...
Future<void> generateFile(
  ...
  if (className == 'AppColors') {
    await _generateColorClass(classBuilder, files, className);
  } else {
    await _generateTypographyClass(classBuilder, files, className);
  }
